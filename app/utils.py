...
def get_consecutive_learning_days(user):
    ...
    try:
        ...
        progress_records = db.find_by_field('learning_progress', 'user_id', user.id)
        if progress_records:
        ...
        submissions = db.find_by_field('submissions', 'user_id', user.id)
        if submissions:
        ...
        for _ in range(max_check_days):
            ...
            elif not found_first and check_date == today:
                check_date -= timedelta(days=1)
            else:
                break
