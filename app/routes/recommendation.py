...
def get_progress():
    ...
    try:
        ...
        accepted = sum(1 for s in submissions if s.status == 'AC')
        wrong_answer = sum(1 for s in submissions if s.status == 'WA')
        runtime_error = sum(1 for s in submissions if s.status == 'RE')
        compile_error = sum(1 for s in submissions if s.status == 'CE')
