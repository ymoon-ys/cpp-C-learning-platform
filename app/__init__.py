...
from flask import Flask, redirect, url_for, render_template, request, flash, send_from_directory, abort, Response
    ...
    if db_type == 'mysql':
        try:
            ...
            if db.conn:
                ...
                print(f'[✗] MySQL 数据库连接失败！自动回退到 SQLite')
                raise RuntimeError('MySQL connection failed')
        except Exception as e:
            print(f'[✗] MySQL 数据库连接失败: {e}')
            print('[→] 自动回退到 SQLite 数据库')
            db_type = 'sqlite'

    if db_type == 'sqlite':
        db_dir = app.config.get('DATABASE_DIR', os.path.join(os.path.dirname(os.path.dirname(__file__)), 'instance'))
    ...
    def uploaded_file(filename):
        ...
        if not os.path.exists(file_path):
            ...
            if folder in DEFAULT_IMAGES:
                ...
                logger.info(f'[UPLOAD] Returning {folder} default image')
                return Response(default_img['data'], mimetype=default_img['mime'])
            else:
                default_img = DEFAULT_IMAGES['default']
                logger.info(f'[UPLOAD] Returning generic default image')
                return Response(default_img['data'], mimetype=default_img['mime'])

        try:
            return send_from_directory(app.config['UPLOAD_FOLDER'], filename)
        except Exception as e:
            logger.error(f'[UPLOAD] File access failed: {file_path}, error: {str(e)}')
            abort(500, description='File access failed')
